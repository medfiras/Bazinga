...
def view_list(request, list_id=0, list_slug=None, view_completed=0):
	...
	return render_to_response('todolist/view_list.html', locals(), context_instance=RequestContext(request))
...
def external_add(request):
	...
	if request.POST:
		...
		if form.is_valid():
			...
			# Send email
			email_subject = render_to_string("todolist/email/assigned_subject.txt", {'task': item.title})
			email_body = render_to_string("todolist/email/assigned_body.txt", {'task': item, 'site': current_site, })
			try:
	...
	return render_to_response('todolist/add_external_task.html', locals(), context_instance=RequestContext(request))
...
def add_list(request):
	...
	return render_to_response('todolist/add_list.html', locals(), context_instance=RequestContext(request))
...
def search(request):
	...
	return render_to_response('todolist/search_results.html', {'query_string': query_string, 'found_items': found_items}, context_instance=RequestContext(request))