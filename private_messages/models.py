...
class MessageManager(models.Manager):
    ...
    def draft_for(self, user):
        """
        Returns all messages that were saved as draft by the given user.
        """
        return self.filter(
            sender=user,
            sender_deleted_at__isnull=True,
            draft_at__isnull=False,
        )
...
class Message(models.Model):
    ...
    draft_at = models.DateTimeField(_("Draft created at"), null=True, blank=True)
    attachment = models.FileField(upload_to='private_messages', verbose_name=_('Attachment'), blank=True, null=True,)
...
def inbox_count_for(user):
    ...
    mark them seen
    """
    return Message.objects.filter(recipient=user, read_at__isnull=True, recipient_deleted_at__isnull=True, draft_at__isnull=True).count()

# fallback for email notification if django-notification could not be found
if "notification" not in settings.INSTALLED_APPS and getattr(settings, 'PRIVATE_MESSAGES_NOTIFY', True):
